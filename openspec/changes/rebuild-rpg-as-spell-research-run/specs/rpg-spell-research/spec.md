...
魔力成本 SHALL 等於行數。1–2 行 SHALL 即時發動；3–4 行 SHALL 需要 1 個詠唱回合；5 行 SHALL 需要 2 個詠唱回合。詠唱期間受到帶 `interrupt` 的敵方意圖命中時，法術 SHALL 失效並退還一半魔力（向下取整）。起手即扣除全部魔力；3–4 行先處理本回合敵方意圖，存活且未被打斷則在同回合立即完成；5 行先處理起手回合意圖，下一回合只能繼續詠唱，處理第二個已揭示意圖後立即完成，不另占第三個完成回合。敵方先造成傷害，再檢查打斷；玩家死亡時不再完成或退魔。打斷命中指敵方未被取消且最終生命傷害大於 0，護盾完全吸收時不打斷。詠唱狀態 SHALL 於戰鬥結束時清除，不跨遭遇保留。系統 SHALL NOT 使用機率失敗率決定施法成敗。
...
首次預覽所需揭示的敵方意圖數 SHALL 等於玩家被要求承諾的回合數：一回合詠唱揭示 1 個意圖，兩回合詠唱揭示 2 個。系統 SHALL NOT 要求玩家在未揭示的意圖上承諾多回合行動。揭示 SHALL 採漸進方式——預設顯示 1 個，玩家考慮需要詠唱的法術時展開對應數量——以避免常態性的資訊過載。玩家已看過但尚未執行的意圖 SHALL 保持可見並持久化；改選短法術、取消預覽或重新載入不得抹去已知資訊。預覽 SHALL 不消耗回合、魔力或亂數，不重新抽取意圖。引擎可在進場及每回合結束時預先抽取有限意圖佇列，但 UI 與搜尋只可取得已揭示部分。只有已配置且目前可合法施放的長法術能展開其承諾範圍，不得藉由未持有或無法施放的法術取得資訊。
...
#### Scenario: Preview a long spell and choose a short one
- **WHEN** 玩家預覽可合法施放的五行法術後改施放一行法術
- **THEN** 已揭示的第二個意圖保留可見，下一回合成為目前意圖
- **AND** 預覽與切換不耗用亂數，讀檔後保留同樣資訊

#### Scenario: Resolve on the last chanting turn
- **WHEN** 五行法術的第二個詠唱回合中玩家存活且未被打斷
- **THEN** 在該回合敵方意圖之後立即結算法術，無須第三個完成回合
- **AND** 3–4 行法術在起手回合敵方意圖之後完成
