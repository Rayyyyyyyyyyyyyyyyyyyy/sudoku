...
| 主要對手 | 3 | 5000/3 | 7500/4 | 10000/5 | 每次 play 恰選一張 |
...
所有表中 target/reward 與 special rule 均標為 `verified`（二手完整實機逐幀），但未宣稱 bit-exact RNG。33 張效果牌的價格、時序與互動多為 `inferred` 或 `unknown`，詳見 versioned catalog 的 `unknownBehavior`。
