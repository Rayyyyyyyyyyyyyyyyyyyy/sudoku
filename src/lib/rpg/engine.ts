...
export function createGame(): GameState {
  return { schemaVersion: 1, contentVersion: CONTENT_VERSION, revision: 0,
    profile: { insight: 0, victories: 0, expeditions: 0, upgrades: [], discoveries: [] }, run: null };
}
...
function apply(run: Run, effects: Effect[]) {
...
export function getAvailableActions(game: GameState): ActionOption[] {
  ...
  return [
    ...
    { id: 'skill', label: spec.skill, detail: `消耗 ${skillCost} 魔力。${spec.skillDescription}`, disabled: run.hero.mana < skillCost, reason: run.hero.mana < skillCost ? '魔力不足，格擋可恢復魔力' : '' },
    { id: 'guard', label: '格擋並調息', detail: `本回合額外減傷 ${getOmen(run.seed).id === 'still-star' ? 10 : 7}，恢復 3 魔力。敵人仍會行動。`, disabled: false, reason: '' },
...
function fight(game: GameState, id: string) {
  ...
  if (id === 'guard') {
    ...
    mitigation = omen.id === 'still-star' ? 10 : 7;
    ...
    if (id === 'skill') {
      ...
      mitigation = hero.classId === 'warrior' ? 0 : 3;
    }
  ...
  const incoming = Math.max(0, intent.damage + (omen.id === 'blood-moon' ? 2 : 0) - hero.armor - mitigation);
...
export function transition(state: GameState, action: Action): TransitionResult {
  ...
  if (action.type === 'start') {
    ...
    game.run = {
      ...
      flags: [], visited: ['village'], battle: null, log: ['遠征開始。觀察敵方招式，保留補給，帶回見聞。'],
    };
  } else if (action.type === 'upgrade') {
